...
Op01:
    ...
    ..Else_If W$esi-1 = 0F901 ; 0F,01,F9 RDTSCP
        inc D$LikelyCode
        add esi 2
        mov D$edi 'rdts', D$edi+4 'cp ' | add edi 7  ; rdtscp
        mov B$DisFlag DISDONE+DISLINEOVER | ret
...
OpF3:
    ..If B$esi = 0F
        .If B$esi+1 = 010       ; F3 0F 10 /r MOVSS xmm1, xmm2/m32
            add esi 2 | call MarkSSEdata SSE_1_R ;  See Op57
            mov D$edi 'movs', W$edi+4 's ' | add edi 6
            jmp Dis_xmm1__xmm2_m32 ; Updated in 20/07/2020
            ;jmp Dis_xmm1__xmm2_m32F
        .Else_If B$esi+1 = 011       ; F3 0F 11 /r MOVSS xmm2/m32, xmm
            add esi 2 | call MarkSSEdata SSE_1_R
            mov D$edi 'movs', W$edi+4 's ' | add edi 6
            ;jmp Dis_xmm2_m32__xmm1 ; Maybe to be used. I didn´ updated in 20/07/2020. Need to review cases of movss edi xmm0 that is crashing on the encodebox
...
L9: On B$WeAreInTheCodeBox = &TRUE, ret ; we are in the encode/decode window. Avoid crashing code like: push D$fs:02*4

    mov ebx D$LastCodeRef | sub ebx D$DisImageBase | add ebx D$UserPeStart
...
WriteApiLabelComment:
    ...
    If B$ConvertIndirectApiCall = &TRUE
        call ConvertDirectApiLabeltoIndirectApiLabel
    Else
        call OutputApiLabelComment
    End_If

ret

[IndirectApiStoredBuffer: B$ 0 #16] ; store the address string (the offset of the PE)

Proc ConvertDirectApiLabeltoIndirectApiLabel:
    uses esi, ecx, eax, ebx
        ...
        push edi
        push eax
            mov edi IndirectApiStoredBuffer
            mov eax D$LastCodeRef
            call WriteEax
            sub edi IndirectApiStoredBuffer
            mov ecx edi
        pop eax
        pop edi
        ...
        mov eax edi
        sub eax 9
        On eax < D$CodeSource, ExitP

        .If_And D$edi-9 = 'call', B$edi-5 = ' ', D$edi-4 = 'Code'
            call CheckForIndirectCall_Jmp
            dec eax | jne L2>
            sub edi 4 ; Pointer to the initial String "Code"
        .Else_If_And D$edi-8 = 'jmp ', D$edi-4 = 'Code'
            call CheckForIndirectCall_Jmp
            dec eax | jne L2>
            sub edi 4 ; Pointer to the initial String "Code"
        .Else
            L2: add edi ecx ; restore the pointer at edi
                ExitP
        .End_If
...
Proc CheckForIndirectCall_Jmp:
    Uses esi, ecx, ebx

    xor eax eax
    mov esi D$LastCodeRef
    sub esi D$DisImageBase | add esi D$UserPeStart  ; SectionsMap

    ; analyse if it is a jmp D$DataXXXXX
    On B$esi <> 0FF, ExitP ; not a possible JMP r/m16 ;  JMP r/m32
    mov eax esi
    inc eax
    mov ecx eax
    inc ecx ; point it to the offset to be computed
    movzx ebx B$eax | DigitMask bl to al
    On al = 4, jmp L2> ; possible jmp r/m16 jmp r/m32 found
    On al = 2, jmp L2> ; possible call r/m16 call r/m32 found
        xor eax eax
        ExitP

        ; check if the pointer is flaged as Code
L2:
    sub esi D$UserPeStart | add esi D$SectionsMap
    If_And D$esi <> ((CODEFLAG shl 24) + (CODEFLAG shl 16) + (CODEFLAG shl 8) + CODEFLAG), W$esi <> ((CODEFLAG shl 8) + CODEFLAG)
        xor eax eax
        ExitP ; all the data chunck is not code, exit.
    End_If
...
L0: add edi 3
    pushad
        ...
        mov D$SpecialFPU 0
        call DisassemblerFloatToUString ST0trash, edi, DISASM_ROUND_MODE
        ...
        .While esi < D$EndOfSectionsMap
            ..If B$esi = TEMPOFLAG
              ; Consider case of Strings Length >= D@length/2 :
                ...
                push ebx | mov ebx esi | sub ebx D$SectionsMap | add ebx D$UserPeStart
                .If_And ecx < D@Length, B$ebx <> 0 ; small fix up. Just jmp if the len is divided by 2 and if the last byte is not 0
                    pop ebx
                    jmp L2>
                .Else_If_And B$esi = 0, B$ebx = 0
                    If ecx <> 4
                        mov B$esi TEMPOFLAG
                    ;Else
                     ;   call IsItPointer;IsPointerCandidate
                      ;  mov eax eax; possible pointer found ?
                    End_If
                .End_If
                pop ebx
        ...
        .While esi < D$EndOfSectionsMap
            ..If B$esi = TEMPOFLAG
              ; Consider case of Strings Length >= D@length/2 :
                ...
                ;If_And ecx < D@Length, W$esi <> 0 ; small fix up. Just jmp if the len is divided by 2 and if the last 2 bytes are not 0
                 ;   jmp L2>
                ;End_If
...
L0:     cmp D$ebx 0 | jne L2>
        ...
        cmp D$ebx+8 0 | jne L2>
            ...
            movzx eax B$esi | cmp al 090 | je L2> ; Op90 nop
                ...
                ; What we want is the whole Dword contain the same value as the 1st byte. Ex: al = 04C, ecx = 04C4C4C4C
                mov ah al | mov ecx eax | shl ecx 16 | or ecx eax
...
L0: .While esi < edx
        ...
        mov ecx D$UserPeEnd | sub ecx 4 ; It cannot be a pointer to the end of the file or smaller then 3 bytes before it´s end. (See: verific-nt.exe)
        .If_And eax >= D$UserPeStart, eax <= ecx
            sub eax D$UserPeStart | add eax D$SectionsMap
            test B$eax IMPORTFLAG+RESOURCESFLAG+EXPORTFLAG+KILLFLAG | jnz L5>>
...
L3:         call 'USER32.GetDlgItemTextA' D@Adressee, 16, HexaCodeText, 80
        ...
        ..Else_If D@wParam = &IDHELP
            call Help D$hwnd, B_U_AsmName, EncodeHelp, ContextHlpMessage
