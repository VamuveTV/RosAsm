...
Proc KillTrailingSpaces:
    ...
    call CorrectCRLFs CodeSource, SourceEnd, SourceLen
...
Proc CorrectCRLFs:
    Arguments @pStart, @pEnd, @pSrcLen
    Local @StartCode
    Uses esi, edi, edx

    mov esi D@pStart | mov esi D$esi | mov D@StartCode esi
    mov edx D@pEnd  | mov edx D$edx
    xor eax eax
    On B$esi =  0, ExitP
        ...
        ..Else_If B$esi = LF
            .If W$esi-2 = CRLF
                If B$esi+1 = 0
                    mov B$esi 0
                    jmp L9>
                Else_If_Or B$esi+1 = CR, B$esi+1 = LF
                    mov W$esi CRLF
                    inc esi
                ;Else
                 ;   mov B$esi CR
...
L9:
    ...
    mov edi D@pSrcLen
    mov eax esi | sub eax D@StartCode | mov D$edi eax
    mov edi D@pEnd | mov D$edi esi
    mov eax &TRUE
EndP
...
[EditorCHOOSEFONT:
 ...
 @iPointSize: D$ 0
 @Flags: D$ &CF_FIXEDPITCHONLY__&CF_SCREENFONTS__&CF_INITTOLOGFONTSTRUCT__&CF_APPLY__&CF_ENABLEHOOK__&CF_NOSIMULATIONS
 @rgbColors: D$ 0
 @lCustData: D$ 0
