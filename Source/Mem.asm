...
Arguments:
                 ...
                 dwBytes: The amount of allocated memory in bytes.

Return Value:
                 If the function succeeds it will return a pointer to allocated memory
                 If the function fails, it returns zero.

Usage:
                  call Operator_New 40

;;

Proc Heap_Operator_New:
    Arguments @dwBytes
    Uses ecx, edx

    call 'KERNEL32.GetProcessHeap'
    call 'KERNEL32.HeapAlloc' eax, &HEAP_ZERO_MEMORY, D@dwBytes

EndP


Proc Heap_Operator_NewEx:
    Arguments @heap, @dwFlags, @dwBytes, @SysPageSize
    Uses ecx, edx, edi

    mov edi D@dwBytes
    If edi <> 0
        Align_On_Variable D@SysPageSize edi
    End_If
    ...
    Else
        mov eax D@heap
    End_If
    call 'KERNEL32.HeapAlloc' eax, D@dwFlags, edi
...
Arguments:
                 ...
                 phMem: Pointer to allocated memory created with Operator_new
...
Usage:
                  call Operator_New 40
                  call Operator_Delete eax
;;

Proc Heap_Operator_Delete:
    Arguments @phMem
    Uses ecx, edx

    If D@phMem <> 0
        call 'KERNEL32.GetProcessHeap'
        call 'KERNEL32.HeapFree' eax, 0, D@phMem
    End_If

EndP


Proc Heap_Operator_DeleteEx:
    Arguments @hHeap, @dwFlags, @lpMem
    Uses ecx, edx
    ...
    End_If
    If D@lpMem <> 0
        call 'KERNEL32.HeapFree' eax, 0, D@lpMem
        ;mov edx D@lpMem | mov D$edx 0
    End_If

EndP

Proc HeapCreateEx:
    Arguments @flOptions, @dwInitialSize, @dwMaximumSize, @SysPageSize
    Uses ecx, edi, esi

    mov esi D@dwInitialSize
    If esi <> 0
        Align_On_Variable D@SysPageSize esi
    End_If

    mov edi D@dwMaximumSize
    If edi <> 0
        Align_On_Variable D@SysPageSize edi
    End_If

    call 'KERNEL32.HeapCreate' D@flOptions, esi, edi

EndP

Proc GetSysPageSize:
    Structure @SysNfo 36, @SysNfo.wProcessorArchitectureDis 0,  @SysNfo.wReservedDis 2,  @SysNfo.dwPageSizeDis 4,  @SysNfo.lpMinimumApplicationAddressDis 8,  @SysNfo.lpMaximumApplicationAddressDis 12,  @SysNfo.dwActiveProcessorMaskDis 16,  @SysNfo.dwNumberOfProcessorsDis 20,  @SysNfo.dwProcessorTypeDis 24,  @SysNfo.dwAllocationGranularityDis 28,  @SysNfo.wProcessorLevelDis 32,  @SysNfo.wProcessorRevisionDis 34
    Uses ecx, edx, esi, edi

    ; 1st determine if the system is using wow64 to we gain accuracy
    call 'KERNEL32.GetModuleHandleA' {B$ "Kernel32", 0}
    call 'KERNEL32.GetProcAddress' eax, {B$ "IsWow64Process", 0}

    If eax <> 0
        call 'kernel32.GetNativeSystemInfo' D@SysNfo
    Else
        call 'kernel32.GetSystemInfo' D@SysNfo
    End_If
    mov eax D@SysNfo.dwPageSizeDis
