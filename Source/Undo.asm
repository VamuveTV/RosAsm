...
[MAX_UNDO_RECORDS 2048]

InitUndo:
    mov D$UndoPtr 0
    mov D$UndoMemory 0
    VirtualAlloc UndoMemory (MAX_UNDO_RECORDS*UNDO_RECORD) | move D$UndoPtr D$UndoMemory
ret
...
;;
    Flag: The type if action to perform. ACTION_INSERT, ACTION_BLOCKDELETE etc
    Param1
    Param2 = Size of the data
;;
...
Proc StoreUserAction:
    ...
    mov edi D$UndoPtr | add edi UNDO_RECORD | mov D$UndoPtr edi ; had an error here. It is edi and not di OutputLe
...
;;
; The new routine don´t crash. Changed 07/08/2018
ReadUndoBlockFile: ; ControlX
    mov D$BlockStartTextPtr ebx, D$BlockEndTextPtr ecx
    ...
    .If eax <> &INVALID_HANDLE_VALUE
        ...
        call 'KERNEL32.GetFileSize' eax, 0 | mov D$UndoBlockLen eax
        add D$SourceLen eax | add D$SourceEnd eax | mov ebx eax
        call StrLenProc D$SourceEnd

        mov esi D$SourceEnd | add esi eax | mov edi esi | add edi ebx
        ;mov esi D$SourceEnd | add esi 400 | mov edi esi | add edi eax
        mov ecx edi | sub ecx D$CurrentWritingPos; | std | rep movsb | cld

        C_call 'msvcrt.memcpy' edi, esi, ecx
...
ControlZ:
    ...
    mov ebx D$UndoPtr | sub ebx UNDO_RECORD ; bx ???
    If ebx < D$UndoMemory ; changed in 07/08/2018 to avoid crashes
        mov ebx D$UndoMemory
    End_If
  ...
  ; >>> eax = RECORD_FLAG, ebx = RECORD_PARAM1, ecx = RECORD_PARAM2
    ...
    .If eax = ACTION_INSERT
        ...
        if B$ebx-1 = LF
            call StripBackSpace
            ;mov ebx D$UndoPtr | sub bx 32 | mov D$UndoPtr ebx ; bx ??
            mov ebx D$UndoPtr | sub ebx 32 | mov D$UndoPtr ebx
            call StripBackSpace
