...
[ManifestList: ? #MAXMANIFEST]      [ManifestListPtr: ManifestList]
...
[FontFilesTitle:     B$ 'Choose a Font File', 0
 ...
 ManifestFilesTitle:   B$ 'Choose a File', 0
 ManifestFilesFilters: B$ 'Manifest Files', 0, '*.xml', 0, 0
...
ReadManifest:
    mov D$OtherFilesFilters ManifestFilesFilters
    mov D$OpenOtherFileTitle ManifestFilesTitle
    mov D$OtherList ManifestList | move D$OtherListPtr D$ManifestListPtr
    call ReadOtherFile
        If D$OtherSaveFilter = 0
            mov D$OtherList 0,  D$OtherListPtr 0 | ret
        End_If
        call AskForResID | add D$ManifestListPtr 12
...
SaveManifest:
    If D$ManifestList = 0
        call 'USER32.MessageBoxA' D$hwnd, NoManifest, Argh, &MB_OK+&MB_SYSTEMMODAL | ret
    End_If

    mov D$WhatDialogListPtr ManifestList,  D$OkDialogFlag &FALSE
   ...
   .End_While
    ...
    call 'USER32.MessageBoxA' D$hwnd, SaveManifestStr, Sure, &MB_ICONQUESTION+&MB_YESNO+&MB_SYSTEMMODAL
   ...
   .If eax = &IDYES
        mov D$OpenDlg.lpstrFilter ManifestFilesFilters
...
[NoManifest: 'No Manifest Resources in this PE', 0]
[KillManifest: 'Delete this Manifest Resource?', 0]
[SaveManifestStr: 'Save this Manifest to Disk?', 0]
[ManifestIdTitle: 'Manifest ID: ' ManifestIdTitleID: '       ', 0]

SetManifestTitle:
    mov eax D$WhatDialogListPtr, eax D$eax-4, edi ManifestIdTitleID
    call TransDwordToAsciiDecimal
    call 'USER32.SendMessageA' D$EmptyDialogHandle &WM_SETTEXT 0 ManifestIdTitle
ret
...
; Tag Dialog 1000
    ...
    .While B$OkDialogFlag = &FALSE
        call SetManifestTitle | call ShowHexa
        call SetNextChoiceID
        call 'User32.DialogBoxIndirectParamA' D$hinstance, ChoiceBar, D$hwnd, ChoiceDialogBoxProc, ManifestList
   ...
   If eax = &IDYES
        ...
        mov eax D$WhatDialogListPtr | sub eax ManifestList | shr eax 2 | mov ecx 300 | sub ecx eax
        rep movsd
        sub D$ManifestListPtr 12
   End_If
