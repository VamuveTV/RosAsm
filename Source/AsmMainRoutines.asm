...
AsmMain:
    ...
    Ros_SaveStack   ; To restore stack on error jump
...
; -------------------------------  First Parsers Jobs ---------------------------------
    ...
    call CoolParsers
;jmp L7>>
    call NewCountStatements | call BarProgress D$ProgressInst, D$WeAreUnfolding
...
L1: call StoreEquatesAndMacros
    ...
    If B$MoreBracket = &TRUE
        ...
        On D$WeAreUnfolding = &TRUE, call UnfoldOutput D$CodeSourceA
        jmp L1<
    End_If

    On D$WeAreUnfolding = &TRUE, ret
...
;;
    ; Important. Resources must be assembled before the dll import, due to some Manifest resources.
    ; If a dll is assembled without a manifest it can have an runtime error R6034
    ; "attempt to load the C runtime library incorrectly..."
    ; More info at: http://msdn.microsoft.com/en-us/library/ms235560(v=vs.80).aspx
    ; Example: msvcr90.dll
    ...
    ; now we are safe to build the import
    call InitIndex2
...
;jmp L7>>
    ...
    If B$ShortenJumpsWanted = &TRUE
        ...
        call EncodeLines D$CodeSourceB
        call JmpsOptimize
    ...
    Else
        call EncodeLines D$CodeSourceB ; Line > Para > Code > op
    End_If
...
;    call CheckCRLFs D$CodeSource, D$SourceLen
