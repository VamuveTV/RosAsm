...
; Modified by Guga
CheckStubMsg:
    ; This is a preparing for the sniff recognition

    mov eax D$UserPeStart | sub eax DosHeader | add eax StubMsg

    mov ecx D$DOS_StubLenght

    .While ecx <> 0

        .If D$eax = 'RosA' ; New RosAsm stub

            L2:
            Do

                If D$eax = 'nt T' ; New RosAsm stub
                    mov D$CompiledBy 'ROSA' | jmp L1>
                Else_If D$eax = ' wiz' ; Old RosAsm Stub
                    mov D$CompiledBy 'ROSA' | jmp L1>
                End_If
            ...
            Loop_Until ecx = 0
            jmp L1>
        .Else_If D$eax = 'spit' ; Old RosAsm Stub
            jmp L2<
        .End_If
...
This values may change if you modify the dos stub msg. Check it at StubMsg
...
;;
  ; For 'DisFail' short exit:
    ...
    call DisInitialise | call FixMzParagraphsNumber D$UserPeStart, D$UserPeLen, DisPeTagPointer
    On eax = &FALSE, ret
...
;map
    ...
    call Kill_Load_Config_Directory

    call KillBound_Import_Directory ; for safety, kill bound imports
...
L1:     inc esi | mov W$esi 'My'
    ...
    .Else
        ...
        mov D$OpenPEStruc+(12*4) OpenPEFileTitle
        call SaveDisPeName UserDisPeName
        SetWindowText
      ...
      ; Progress Bar:
        If edi > D$NextDisBarPos
            push edi
                ...
                call BarProgress D$ProgressInst, D$WeAreUnfolding
            pop edi
        End_If
    End_While
...
StartNewDisFile:
    ;Get PEHeaderPointer and remark the PEORigin
    mov eax D$DisPeTagPointer; DisPeOrigine
    sub eax D$UserPeStart
    sub eax D$PeHeaderPointer
    mov D$DisPeOrigine eax
...
;;
  ...
  If either 'Import LookUp Table RVA (OriginalFirstThunk)' or 'Import Addresses Table RVA (FirstThunk)' are zeroed,
  we fill the one by the other, in order to save from no end Checking:
;;


    mov eax edx ; points to Import_Table_Adress
    xor ecx ecx ; our counter of array structures

    ; We need to go through all the array of IMAGE_IMPORT_DESCRIPTOR untill we found the Import Directory Terminator
L0:

    If_And D$eax+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis = 0, D$eax+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis = 0,
           D$eax+IMAGE_IMPORT_DESCRIPTOR.TimeDateStampDis = 0, D$eax+IMAGE_IMPORT_DESCRIPTOR.ForwarderChainDis = 0,
           D$eax+IMAGE_IMPORT_DESCRIPTOR.Name1Dis = 0, D$eax+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis = 0
        jmp L1> ; If we found Import Directory Terminator End the Looping
    Else_If D$eax+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis = 0
        move D$eax+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis D$eax+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis
    Else_If D$eax+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis = 0
        move D$eax+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis D$eax+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
    End_If

    ; Check if it was compiled by VB.
    push edx
        mov edx D$eax+IMAGE_IMPORT_DESCRIPTOR.Name1Dis | add edx D$UserPeStart
        On D$edx = 'MSVB', mov D$CompiledBy 'MSVB'
    pop edx

    add eax Size_Of_IMAGE_IMPORT_DESCRIPTOR
    inc ecx
    jmp L0< ; Jmp Back to the next array of the structure
...
L1:
    ...
    push edx
        ...
        ; Flag all the array of IMAGE_IMPORT_DESCRIPTOR structures including the Import Directory Terminator
        push ecx
            mov ebx ecx
            inc ebx ; include the Import Directory Terminator

            Do
                mov ecx 5 ; each IMAGE_IMPORT_DESCRIPTOR structure contains 5 Dwords
                rep stosd ; fill each structure with IMPORTFLAG (All 5 Dwords)
                dec ebx
            Loop_Until ebx = 0

        pop ecx

        ; now we flag the contents of OriginalFirstThunk (ImportLookUpTable), Name1 , FirstThunk

        ..While ecx <> 0
            call Flag_ImportLookUpTable edx, IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
            call Flag_NameRVA edx, IMAGE_IMPORT_DESCRIPTOR.Name1Dis
            call Flag_FirstThunk edx
            dec ecx ; decrease the array counter
            add edx Size_Of_IMAGE_IMPORT_DESCRIPTOR
        ..End_While

    pop edx

;;
  Once done, when we find a Pointer, and see 'IMPORTFLAG' in the relative 'SectionsMap'
...
L0:     mov esi D$edx+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis | add esi D$UserPeStart
        While D$esi <> 0
            add esi 4 | inc D$DisNumberOfFunctions
        End_While

        add edx Size_Of_IMAGE_IMPORT_DESCRIPTOR | cmp D$edx+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis 0 | jne L0<<

    pop edx, esi

    ; Gte numbers of all possible delayed Import Functions if existant (See CheckDelayImport )
    call GetNumberOfDelayedFunctions
    mov ecx eax

  ; Large estimation: 128 Chars per Function:
    mov eax D$DisNumberOfFunctions | add eax ecx | shl eax 7
    pushad
        push eax
  ...
  ; Now, fill the ApiBuffer:
    ...
    mov ebx D$edx+IMAGE_IMPORT_DESCRIPTOR.Name1Dis | add ebx D$UserPeStart    ; Pointer to DLL Name.
    mov esi D$edx+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis | add esi D$UserPeStart    ; Pointer to Functions Names Address Table.
        ...
        While D$esi <> 0
            ...
            ; 'DllName.
            push ebx
                Do | mov al B$ebx | stosb | inc ebx | Loop_Until al = 0
                dec ebx | dec edi

                ; The import can have whatever extension and not only ".dll"
                Do | dec edi | Loop_Until B$edi = '.'
                inc edi ; bypass  the '.'
            pop ebx

            lodsd ; get the value from the IAT (IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis)

            ; Check if it is imported by ordinal or by name.
            Test_If eax 08000_0000 ; Imported by Ordinal &IMAGE_ORDINAL_FLAG
                xor eax 08000_0000 | push ebx | call WriteEax | pop ebx
            Test_Else
                push ebx
                    mov ebx eax | add ebx D$UserPeStart | add ebx 2
                    ; ....FunctionName'
                    Do
                        mov al B$ebx | stosb | inc ebx
                    Loop_Until al = 0
                    dec edi
                pop ebx
            Test_End

            mov B$edi "'" | inc edi | mov B$edi 0 | inc edi
;;
            ; Overwrite Address Table and LookUp Table dWords by Pointers to 'ApiBuffer':
            mov D$esi-4 ecx ; esi-4 = The IAT content. Copy the address of the dll name to it.
            ; do the same, but copy it to ILT
            mov eax esi
            sub eax D$edx+(4*4)
            add eax D$edx+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
            mov D$eax-4 ecx
;;
            ; Overwrite Address Table and LookUp Table dWords by Pointers to 'ApiBuffer':
            mov D$esi-4 ecx ; esi-4 = The IAT content. Copy the address of the dll name to it.
            ; do the same, but copy it to ILT
            mov eax esi
            sub eax D$edx+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis
            add eax D$edx+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
            mov D$eax-4 ecx
    ...
    mov D$End_Of_ApiBuffer_String edi ; to be used in CheckDelayImport

    call KillBlankBytes AppImportSize, IMPORTFLAG
ret
_________________________________________________________

Proc Flag_ImportLookUpTable:
    Argument @Addresse, @Equate ; pointer to the offset to IMAGE_IMPORT_DESCRIPTOR

    pushad

    mov edx D@Addresse
    mov ecx D@Equate
    mov edi D$edx+ecx;IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
    If ecx = IMAGE_DELAY_DESCRIPTOR.pINTDis
        sub edi D$DisImageBase
    End_If
    mov ebx edi
    ...
    .While D$ebx <> 0
        ...
        ; flag the Dword
        mov D$edi IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)

        .Test_If_Not D$ebx 08000_0000 ; If it is imported by name, flag it´s pointers
;;
...
;;
            push edi
                ...
                add eax 2 ; bypass the 1st Word
                mov W$edi IMPORTFLAG+(IMPORTFLAG shl 8)
                add edi 2

                Do
                    mov B$edi IMPORTFLAG
                    inc edi
                    inc eax
                Loop_Until B$eax = 0 ; Flag the whole null terminated string
                mov B$edi IMPORTFLAG ; Flag the null terminated byte
            pop edi

        .Test_End

        add ebx 4 ; point to the next dword
        add edi 4 ; point to the next dword on the SectionMap
    .End_While

     ; Flag the Null Terminated Dword
     mov D$edi IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
...
; the amount of Bound IAT is the same of the IAT, plus the null terminator dword
; eax dword may points to a funtion related to the IMAGE_BOUND_IMPORT_DESCRIPTOR found in BoundIAT_Directory_Structures

Proc Flag_pBoundIAT:
    Argument @Addresse ; pointer to the offset to IMAGE_IMPORT_DESCRIPTOR

    pushad

    mov edx D@Addresse
    mov ecx D$IATCount
    inc ecx ; include the null terminator dword

    mov edi D$edx+IMAGE_DELAY_DESCRIPTOR.pBoundIATDis
    On edi = 0, jmp L1>

    If D$edx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis = 0
        sub edi D$DisImageBase
    End_If
...
Proc Flag_pUnLoadIAT:
    ...
    mov edi D$edx+IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis
    On edi = 0, jmp L1>
...
Proc Flag_NameRVA:
    ...
    pushad
        ...
        mov edx D@Addresse
        mov ecx D@Equate
        mov edi D$edx+ecx;IMAGE_IMPORT_DESCRIPTOR.Name1Dis

        .If edi <> 0

            If ecx = IMAGE_DELAY_DESCRIPTOR.szNameDis
                sub edi D$DisImageBase
            End_If
            ...
            Do
                ...
                inc edi
                inc ebx
            Loop_Until B$ebx = 0 ; Flag the whole null terminated string
            mov B$edi IMPORTFLAG ; Flag the null terminated byte
...
Proc Flag_FirstThunk:
    ...
    pushad
        ...
        mov edx D@Addresse
        mov esi D$edx+IMAGE_IMPORT_DESCRIPTOR.OriginalFirstThunkDis
        mov edi D$edx+IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis
        mov eax edi
        mov ebx edi
        add edi D$SectionsMap | add ebx D$UserPeStart

        ; Flag the array of Pointers until we found the Null Terminated Dword
        While D$ebx <> 0
            ; flag the Dword
            mov D$edi IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
            add edi 4
        ...
        ; The IAT will be a copy of the Import LookUp Table found from Flag_ImportLookUpTable
        add esi D$UserPeStart | add eax D$UserPeStart
        mov edi eax
        While D$esi <> 0
            movsd
        End_While
...
; See Load_Config_Directory_Structures for more information

Kill_Load_Config_Directory:

    GetPeHeader PeHeader.DataDirectory.ConfigTable | On D$eax = 0, ret

    mov ecx D$eax+4
    On ecx = 0, ret
    mov eax D$eax | mov edi eax | add eax D$UserPeStart

    add edi D$SectionsMap
    push ecx
    push eax
        mov al KILLFLAG | rep stosb
    pop eax
    pop ecx
    ; now we follow the proper pointers
    .If ecx = Size_Of_IMAGE_LOAD_CONFIG_DIRECTORY_OLD
        mov esi D$eax+IMAGE_LOAD_CONFIG_DIRECTORY_OLD.SecurityCookieDis
        sub esi D$DisImageBase
        add esi D$SectionsMap ; flag the SecurityCookie pointed
        mov D$esi DATAFLAG+(DATAFLAG shl 8)+(DATAFLAG shl 16)+(DATAFLAG shl 24)

        ; now flag th SEH Pointers tha are simply pointers to functions
        mov esi D$eax+IMAGE_LOAD_CONFIG_DIRECTORY_OLD.SEHandlerTableDis
        ;mov edx esi
        ...
        mov ecx D$eax+IMAGE_LOAD_CONFIG_DIRECTORY_OLD.SEHandlerCountDis

        While ecx <> 0

            mov D$esi KILLFLAG+(KILLFLAG shl 8)+(KILLFLAG shl 16)+(KILLFLAG shl 24)
            add esi 4 ; point to the next array

            mov ebx D$edx
            If ebx <> 0
                mov edi ebx
                add ebx D$SectionsMap
                add edi D$RoutingMap
                mov B$ebx CODEFLAG
                mov B$edi INSTRUCTION+EVOCATED+LABEL+ACCESSED+PUSH_EBP
            End_If
            add edx 4
            dec ecx
        End_While

        mov esi D$eax+IMAGE_LOAD_CONFIG_DIRECTORY_OLD.LockPrefixTableDis
        mov ebx D$eax+IMAGE_LOAD_CONFIG_DIRECTORY_OLD.EditListDis

    .Else_If ecx = Size_Of_IMAGE_LOAD_CONFIG_DIRECTORY

        mov esi D$eax+IMAGE_LOAD_CONFIG_DIRECTORY.LockPrefixTableDis
        mov ebx D$eax+IMAGE_LOAD_CONFIG_DIRECTORY.EditListDis

    .Else_If ecx = Size_Of_IMAGE_LOAD_CONFIG_DIRECTORY64 ; Need further analysis for 64Bit apps.

        mov esi D$eax+IMAGE_LOAD_CONFIG_DIRECTORY64.LockPrefixTableDis
        mov ebx D$eax+IMAGE_LOAD_CONFIG_DIRECTORY64.EditListDis
    ...
    If esi <> 0
        sub esi D$DisImageBase
        add esi D$SectionsMap
        mov D$esi DATAFLAG+(DATAFLAG shl 8)+(DATAFLAG shl 16)+(DATAFLAG shl 24) ; ? probably a pointer
    End_If

    If ebx <> 0
        sub ebx D$DisImageBase
        add ebx D$SectionsMap
        mov D$ebx DATAFLAG+(DATAFLAG shl 8)+(DATAFLAG shl 16)+(DATAFLAG shl 24) ; ? probably a pointer
    End_If
...
KillBound_Import_Directory_Old:

    GetPeHeader PeHeader.DataDirectory.BoundIAT | On D$eax = 0, ret
    mov ecx D$eax+4 ; ecx = BoundIATSize
    On ecx = 0, ret
    mov eax D$eax | mov edi eax | add eax D$UserPeStart

    add edi D$SectionsMap | mov al KILLFLAG | rep stosb
ret

KillBound_Import_Directory:
    ...
    ; We need to go through all the array of IMAGE_BOUND_IMPORT_DESCRIPTOR until we found the Delay-Load Directory Terminator
L0:

    If_And  D$eax+IMAGE_BOUND_IMPORT_DESCRIPTOR.TimeDateStampDis = 0, W$eax+IMAGE_BOUND_IMPORT_DESCRIPTOR.OffsetModuleNameDis = 0,
            W$eax+IMAGE_BOUND_IMPORT_DESCRIPTOR.NumberOfModuleForwarderRefsDis = 0
        jmp L1> ; Delay-Load Directory Terminator End the Looping
    End_If

    ;mov ecx KILLFLAG+(KILLFLAG shl 8)+(KILLFLAG shl 16)+(KILLFLAG shl 24)
    mov ecx IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
    mov D$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.TimeDateStampDis ecx
    mov W$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.OffsetModuleNameDis cx
    mov W$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.NumberOfModuleForwarderRefsDis cx

    ; now we flag the null terminated string
    push ecx
        movzx ecx W$eax+IMAGE_BOUND_IMPORT_DESCRIPTOR.OffsetModuleNameDis
        add ecx esi

        push edi
            mov edi ecx
            sub edi D$UserPeStart
            add edi D$SectionsMap
            ...
            Do
                ...
                inc edi
                inc ecx
            Loop_Until B$ecx = 0 ; Flag the whole null terminated string
            mov B$edi IMPORTFLAG ; Flag the null terminated byte
    ...
    add eax Size_Of_IMAGE_BOUND_IMPORT_DESCRIPTOR
    add edi Size_Of_IMAGE_BOUND_IMPORT_DESCRIPTOR
...
L1:
    ...
    ; Flag the null terminated IMAGE_BOUND_IMPORT_DESCRIPTOR
    mov ecx IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
    mov D$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.TimeDateStampDis ecx
    mov W$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.OffsetModuleNameDis cx
    mov W$edi+IMAGE_BOUND_IMPORT_DESCRIPTOR.NumberOfModuleForwarderRefsDis cx
...
[Delay_grAttrs_RVA: D$ 0]


Proc CheckDelayImport:
    Local @hApiData

;;
    GetPeHeader PeHeader.DataDirectory.DelayID | On D$eax = 0, ret
    mov ecx D$eax+4
    On ecx = 0, ret
    mov eax D$eax | mov edi eax | add eax D$UserPeStart

    add edi D$SectionsMap | mov al IMPORTFLAG | rep stosb
;;

    GetPeHeader PeHeader.DataDirectory.DelayIDSize | mov eax D$eax | On eax = 0, ExitP
    GetPeHeader PeHeader.DataDirectory.DelayID | mov edx D$eax | On edx = 0, jmp DisFail

    add edx D$UserPeStart

    mov eax edx ; points to Delay_Import_Address_Table_Structures
    xor ecx ecx ; our counter of array structures

    ; We need to go through all the array of IMAGE_DELAY_DESCRIPTOR until we found the Delay-Load Directory Terminator
L0:

    If_And  D$eax+IMAGE_DELAY_DESCRIPTOR.grAttrsDis = 0, D$eax+IMAGE_DELAY_DESCRIPTOR.szNameDis = 0,
            D$eax+IMAGE_DELAY_DESCRIPTOR.phmodDis = 0, D$eax+IMAGE_DELAY_DESCRIPTOR.pIATDis = 0,
            D$eax+IMAGE_DELAY_DESCRIPTOR.pINTDis = 0, D$eax+IMAGE_DELAY_DESCRIPTOR.pBoundIATDis = 0,
            D$eax+IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis = 0, D$eax+IMAGE_DELAY_DESCRIPTOR.dwTimeStampDis = 0
        jmp L1> ; Delay-Load Directory Terminator End the Looping
    Else_If D$eax+IMAGE_DELAY_DESCRIPTOR.pINTDis = 0
        move D$eax+IMAGE_DELAY_DESCRIPTOR.pINTDis D$eax+IMAGE_DELAY_DESCRIPTOR.pIATDis
    Else_If D$eax+IMAGE_DELAY_DESCRIPTOR.pIATDis = 0
        move D$eax+IMAGE_DELAY_DESCRIPTOR.pIATDis D$eax+IMAGE_DELAY_DESCRIPTOR.pINTDis
    End_If

    ; Check and fix possible attributespointers
    call CheckAttributes_grAttrs eax

    ; Check if it was compiled by VB.
    push edx
        ;mov edx D$eax+IMAGE_IMPORT_DESCRIPTOR.Name1Dis | add edx D$UserPeStart HERE have an error on winremote
        mov edx D$eax+IMAGE_DELAY_DESCRIPTOR.szNameDis | sub edx D$DisImageBase | add edx D$UserPeStart
        ;mov edx D$eax+IMAGE_DELAY_DESCRIPTOR.szNameDis | add edx D$UserPeStart
        On D$edx = 'MSVB', mov D$CompiledBy 'MSVB'
    pop edx

    add eax Size_Of_IMAGE_DELAY_DESCRIPTOR
...
L1:
    ...
    push edx
        ...
        ; Flag all the array of IMAGE_DELAY_DESCRIPTOR structures including the Delay-Load Directory Terminator
        push ecx
            mov ebx ecx
            inc ebx ; include the Import Directory Terminator

            Do
                mov ecx 8 ; each IMAGE_DELAY_DESCRIPTOR structure contains 8 Dwords
                rep stosd ; fill each structure with IMPORTFLAG (All 8 Dwords)
                dec ebx
            Loop_Until ebx = 0
        ...
        ; now we flag the contents of OriginalFirstThunk (ImportLookUpTable), Name1 , FirstThunk
;;
[IMAGE_DELAY_DESCRIPTOR.grAttrsDis 0 ok
 IMAGE_DELAY_DESCRIPTOR.szNameDis 4 ok
 IMAGE_DELAY_DESCRIPTOR.phmodDis 8 ok
 IMAGE_DELAY_DESCRIPTOR.pIATDis 12 ok
 IMAGE_DELAY_DESCRIPTOR.pINTDis 16 ok
 IMAGE_DELAY_DESCRIPTOR.pBoundIATDis 20
 IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis 24
 IMAGE_DELAY_DESCRIPTOR.dwTimeStampDis 28]
;;
        ..While ecx <> 0
            ;call CheckAttributes_grAttrs edx
            move D$Delay_grAttrs_RVA D$edx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis
            call Flag_NameRVA edx, IMAGE_DELAY_DESCRIPTOR.szNameDis
            ; Flag Module module handle
            call Flag_ModuleHandle edx
            call Flag_pIAT edx
            call Flag_ImportLookUpTable edx, IMAGE_DELAY_DESCRIPTOR.pINTDis
            call Flag_pBoundIAT edx ; IMAGE_DELAY_DESCRIPTOR.pBoundIATDis
            call Flag_pUnLoadIAT edx ; IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis
            dec ecx ; decrease the array counter
            add edx Size_Of_IMAGE_DELAY_DESCRIPTOR
        ..End_While
...
;;
    push esi, edx
      ...
      ; Pointer to Functions Names Address Table in edx:
L0:
        mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis | sub esi D$DisImageBase | add esi D$UserPeStart
        While D$esi <> 0
            add esi 4 | inc D$DisNumberOfFunctions
        End_While

        add edx Size_Of_IMAGE_DELAY_DESCRIPTOR | cmp D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis 0 | jne L0<<

    pop edx, esi


  ; Now, fill the ApiBuffer:
    mov edi D$End_Of_ApiBuffer_String

    .Do

        move D$Delay_grAttrs_RVA D$edx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis
        mov ebx D$edx+IMAGE_DELAY_DESCRIPTOR.szNameDis | sub ebx D$DisImageBase | add ebx D$UserPeStart    ; Pointer to DLL Name.
        mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis | sub esi D$DisImageBase | add esi D$UserPeStart    ; Pointer to Functions Names Address Table.

        mov eax D$edx+IMAGE_DELAY_DESCRIPTOR.pIATDis | sub eax D$DisImageBase | add eax D$UserPeStart
        mov ecx esi
        ...
        While D$esi <> 0
                ...
                ;mov ecx edi
                mov D@hApiData edi
                call WriteDll_FunctionName esi, edi, ebx, &TRUE

                ; Overwrite Address Table and LookUp Table dWords by Pointers to 'ApiBuffer':
                move D$eax D@hApiData
                move D$ecx D@hApiData
                add esi 4
                add eax 4
                add ecx 4
...
Proc CheckAttributes_grAttrs:
    Argument @Addresse; pointer to the offset to IMAGE_IMPORT_DESCRIPTOR

    pushad

        mov edx D@Addresse
        mov edi D$edx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis

        .If edi = 1
            mov ecx D$DisImageBase
            On D$edx+IMAGE_DELAY_DESCRIPTOR.szNameDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.szNameDis ecx
            On D$edx+IMAGE_DELAY_DESCRIPTOR.phmodDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.phmodDis ecx
            On D$edx+IMAGE_DELAY_DESCRIPTOR.pIATDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.pIATDis ecx
            On D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis ecx
            On D$edx+IMAGE_DELAY_DESCRIPTOR.pBoundIATDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.pBoundIATDis ecx
            On D$edx+IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis <> 0, add D$edx+IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis ecx
        .End_If
...
Proc WriteDll_FunctionName:
    Arguments @Input, @Output, @DLLName, @IsDelayDescriptor
    uses eax, esi
    ...
    mov ebx D@DLLName
            ...
            ; Check if it is imported by ordinal or by name.
            Test_If eax 08000_0000 ; Imported by Ordinal
                xor eax 08000_0000 | push ebx | call WriteEax | pop ebx
            Test_Else
                push ebx
                    ;mov ebx eax | add ebx D$UserPeStart | add ebx 2
                    mov ebx eax
                    .If D@IsDelayDescriptor = &TRUE
                        If D$Delay_grAttrs_RVA = 0
                            sub ebx D$DisImageBase
                        End_If
                    .End_If
                    add ebx D$UserPeStart
                    add ebx 2
...
Proc GetNumberOfDelayedFunctions:
    Uses edx, esi

    GetPeHeader PeHeader.DataDirectory.DelayIDSize | mov eax D$eax | On eax = 0, ExitP
    GetPeHeader PeHeader.DataDirectory.DelayID | mov edx D$eax
    If edx = 0
        xor eax eax | ExitP
    End_If
    ...
    xor eax eax
      ...
      ; Pointer to Functions Names Address Table in edx:
L0:     ;mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis | add esi D$UserPeStart
        ;mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis | sub esi D$DisImageBase | add esi D$UserPeStart
        ;mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis | add esi D$UserPeStart
        mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis

        If D$edx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis = 1
...
Proc Flag_ModuleHandle:
    Argument @Addresse; pointer to the offset to IMAGE_DELAY_DESCRIPTOR
    Uses edx, eax

    mov edx D@Addresse
    mov eax D$edx+IMAGE_DELAY_DESCRIPTOR.phmodDis
    on eax = 0, ExitP
    sub eax D$DisImageBase
    add eax D$SectionsMap
    ; The pointer is only used by Module Handle, we can Kill it
    mov D$eax KILLFLAG+(KILLFLAG shl 8)+(KILLFLAG shl 16)+(KILLFLAG shl 24)

EndP

____________________________________________________________________________________________

[IATCount: D$ 0] ; the amount of IAT (DLL functions) found per Dll. (To be used in Flag_pBoundIAT)
...
Proc Flag_pIAT:
    ...
    mov edx D@Addresse
    mov edi D$edx+IMAGE_DELAY_DESCRIPTOR.pIATDis
    sub edi D$DisImageBase
    mov ebx edi
    add edi D$UserPeStart
    add ebx D$SectionsMap

    ;call KIllDelayedFunctions edi

    ; Replace the Data Existant in pIAt (That are pointers to code) with the data found in IMAGE_IMPORT_DESCRIPTOR.FirstThunkDis

    ; esi must point to ILT
    mov esi D$edx+IMAGE_DELAY_DESCRIPTOR.pINTDis
    sub esi D$DisImageBase
    add esi D$UserPeStart
    ...
    Do
        ...
        inc D$IATCount
        ; 1st we flag the pointer
        mov D$ebx IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
        add ebx 4

        ; Kill the delayed functions
        call KillDelayedFunctions edi

        mov eax D$esi ; eax = the content of esi (The IAT data)
        If D$Delay_grAttrs_RVA <> 0
            add eax D$DisImageBase
        End_If
        mov D$edi eax
        add esi 4
        add edi 4
    ...
    ; and flag the null terminated Dword
    mov D$ebx IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)
...
EndP
____________________________________________________________________________________________
; this crap mainly is found on user32.dll
;;

http://msdn.microsoft.com/pt-br/library/2b054ds4.aspx
http://msdn.microsoft.com/pt-br/library/8yfshtha.aspx
http://msdn.microsoft.com/pt-br/library/151kt790.aspx
 ...
 If you have multiple versions of Visual C++ on your computer, make sure the linker matches delayimp.lib.
 If there is a mismatch, you will get a linker error reporting either ___delayLoadHelper2@8 or ___delayLoadHelper@8 as an unresolved external symbol.
 The former implies a new linker with an old delayimp.lib, and the latter implies an old linker with a new delayimp.lib.

If you get an unresolved linker error, run dumpbin /linkermember:1 on the delayimp.lib that you expect to contain the helper function to see which helper
function is defined instead. The helper function could also be defined in an object file; run dumpbin /symbols and look for delayLoadHelper(2).
...
If you defined your own helper function and are using the current version of Visual C++, do the following:
    ...
    Since the pointers in the delay descriptor (ImgDelayDescr in delayimp.h) have been changed from absolute addresses (VAs) to relative addresses (RVAs) to work
    as expected in both 32- and 64-bit programs, you need to convert these back to pointers. A new function has been introduced: PFromRva, found in delayhlp.cpp.
    You can use this function on each of the fields in the descriptor to convert them back to either 32- or 64-bit pointers.
    The default delay load helper function continues to be a good template to use as an example.
...
Proc KillDelayedFunctions:
    ...
    call IsItDelayFunction01 esi
    If eax = &TRUE
        call KillDelayFunction01 esi
        ExitP
    End_If

    call IsItDelayFunction02 esi
    If eax = &TRUE
        mov eax eax
    End_If

    call IsItDelayFunction03 esi ; 179 bytes
    If eax = &TRUE
        call KillDelayFunction03 esi
        ExitP
    End_If
...
Proc IsItDelayFunction01:
    ...
    ; 1st check the contents of the functions
;;
    Is it the code below ?
Code076387FB5: L7:
    mov eax Data0763B1000             ; B8 00 10 3B 76 Pointer to the IMAGE_DELAY_DESCRIPTOR.pIATDis (Must already be flkagged as IMPORT)
    jmp Code0763829DC                 ; E9 1D AA FF FF 

    Or, also the short jmp
    
Code0763ADEB4: O8:
    mov eax Data0763B1028             ; B8 28 10 3B 76  Pointer to the IMAGE_DELAY_DESCRIPTOR.pIATDis (Must already be flkagged as IMPORT)
jmp J2<  ; Code0763ADE7C              ; EB C1

;;

    On B$esi <> 0B8, ExitP ; This is not the code, exit
    mov ecx D$esi+1 ; See if 00 10 3B 76  is a pointer to the Delay Import Data
    sub ecx D$DisImageBase
    add ecx D$SectionsMap
    ; If the pointer was not flaged with IMPORT, exit
    On D$ecx <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24), ExitP
    add esi 5 ; Bypass the identifier byte (0B8) and the next dword (0763B1000)
    ; now we are in jmp Code0763829DC

    ; Go to the address of the next code
    .If B$esi = 0E9
        mov ecx D$esi+1 ; bypass the byte
        If ecx s 0
            add ecx 5 ; go to the end of the jmp Code0763829DC (E9 1D AA FF FF) we are at the last 0FF
            neg ecx
            sub esi ecx
        Else
            add ecx 5 ; go to the end of the jmp Code0763829DC (E9 1D AA FF FF) we are at the last 0FF
    ...
    .Else_If B$esi = 0EB
        movzx ecx B$esi+1 ; bypass the byte
        mov edx ecx

        ; Try finding short jmp
        and ecx 00_1000_0000
        add esi 2
    ...
    .End_If
;;
    On B$esi <> 0E9, ExitP ; If it is not a jmp CodeXXXXX, exit
    mov ecx D$esi+1 ; bypass the byte
...
Code0763829DC: G0:
    ...
    push edx                          ; 52 
    push eax                          ; 50 
    push Data0763AEC34                ; 68 34 EC 3A 76 Pointer to the start of IMAGE_DELAY_DESCRIPTOR
    call Code076382A3C                ; E8 53 00 00 00 ; The main delayLoadHelper function
    pop edx                           ; 5A 
    pop ecx                           ; 59 
    jmp eax                           ; FF E0 
...
;;
    ...
    On D$esi <> 068505251, ExitP ; (push ecx + push edx + push eax + 1st byte of the DwordPointer)
    add esi 4
    mov ecx D$esi ; get the Data0763AEC34 and see if the address points to a IMAGE_DELAY_DESCRIPTOR (Flagged all with IMPORTFLAG)
    sub ecx D$DisImageBase
    add ecx D$SectionsMap

    If_And D$ecx+IMAGE_DELAY_DESCRIPTOR.grAttrsDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.szNameDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.phmodDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.pIATDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.pINTDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.pBoundIATDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.pUnloadIATDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24),
            D$ecx+IMAGE_DELAY_DESCRIPTOR.dwTimeStampDis <> IMPORTFLAG+(IMPORTFLAG shl 8)+(IMPORTFLAG shl 16)+(IMPORTFLAG shl 24)

        ExitP

    End_If

    add esi 4 ; Bypass the Data0763AEC34

    If_And B$esi <> 0E8, D$esi+5 <> 0E0FF595A ; (1st byte from call Code076382A3C + pop edx + pop ecx + jmp eax)
        ExitP
    End_If

    ; Follow call Code076382A3C

    mov ecx D$esi+1 ; Bypass the 0E8 (The 1st byte of the call instruction
    ; Go to the address of the next code

    If ecx s 0 ; Is ecx signed ? (Negative number)
        add ecx 5 ; go to the end of the call Code076382A3C ; E8 53 00 00 00 we are at the last 0FF
        neg ecx
        sub esi ecx
    Else
        add ecx 5 ; go to the end of the call Code076382A3C ; E8 53 00 00 00 we are at the last 0FF
...
Code076382A3E: P8:
    push ebp                          ; 55 
    mov ebp esp                       ; 8B EC 
    sub esp 028                       ; 83 EC 28 
    mov eax D$ebp+08                  ; 8B 45 08 
    push ebx                          ; 53 
    mov ebx D$eax+04                  ; 8B 58 04 
    push esi                          ; 56 
    mov esi D$eax+08                  ; 8B 70 08 
    push edi                          ; 57 
    mov edi D$ebp+0C                  ; 8B 7D 0C 
    sub edi D$eax+0C                  ; 2B 78 0C 
    mov eax D$eax+010                 ; 8B 40 10 
    mov ecx Data076380000                 ; B9 00 00 38 76 Points to the Start of the File 'MZ' DosHeader.e_magic
    sub edi ecx                       ; 2B F9 
    sar edi 02                        ; C1 FF 02 
    add esi ecx                       ; 03 F1 
    mov edx D$esi                     ; 8B 16 
    add ebx ecx                       ; 03 D9 
    lea eax D$eax+edi*4+Data076380000     ; 8D 84 B8 00 00 38 76  Points to the Start of the File 'MZ' DosHeader.e_magic
    mov ecx D$eax                     ; 8B 08 
    test ecx ecx                      ; 85 C9 
     | js Code076383C11               ; 0F 88 97 11 00 00 
    lea eax D$ecx+Data076380002       ; 8D 81 02 00 38 76 Points to DosHeader.e_cblp
    
Code076382A80: G4:
    test edx edx                      ; 85 D2 
    mov D$ebp-04 eax                  ; 89 45 FC 
     | je Code076383BBA               ; 0F 84 2F 11 00 00 
    
Code076382A8B: H5:
    push D$ebp-04                     ; FF 75 FC 
    push edx                          ; 52 
    call 'KERNEL32.GetProcAddress'    ; FF 15 A8 10 38 76 
    test eax eax                      ; 85 C0 
     | je Code076383C19               ; 0F 84 7C 11 00 00 
    
Code076382A9D: J3:
    mov ecx D$ebp+0C                  ; 8B 4D 0C 
    pop edi                           ; 5F 
    pop esi                           ; 5E 
    mov D$ecx  eax                    ; 89 01 
    pop ebx                           ; 5B 
...
;;
   ...
   ; 1st check the initial data

    .If_And D$esi <> 08B55FF8B, D$esi+4 <> 028EC83EC, D$esi+8 <> 05308458B, D$esi+12 <> 05604588B, D$esi+16 <> 05708708B,
            D$esi+20 <> 02B0C7D8B, D$esi+24 <> 0408B0C78, W$esi+28 <> 0B910, D$esi+34 <> 0FFC1F92B, D$esi+38 <> 08BF10302,
            D$esi+42 <> 08DD90316, W$esi+46 <> 0B884, D$esi+52 <> 0C985088B, D$esi+56 <> 01197880F, D$esi+60 <> 0818D0000,
            D$esi+68 <> 04589D285, D$esi+72 <> 02F840FFC, D$esi+76 <> 0FF000011, D$esi+80 <> 0FF52FC75, B$esi+84 <> 015,
            D$esi+89 <> 0840FC085, D$esi+93 <> 0117C, D$esi+97 <> 05F0C4D8B, D$esi+101 <> 05B01895E, D$esi+105 <> 08C2C9
        ExitP
    .End_If

    ; Now we check the pointers
    mov ecx D$esi+30 ; offset image_base
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase | add ecx D$UserPeStart
    On W$ecx = 'MZ', jmp L1> ; Is it DosHeader.e_magic ?
    On W$ecx = 'ZM', jmp L1> ; Is it the older version of DosHeader.e_magic ?
        ExitP ; This is not the DosHeader.e_magic, Exit
    L1:

    mov ecx D$esi+48 ; offset image_base,
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase | add ecx D$UserPeStart
    On W$ecx = 'MZ', jmp L1> ; Is it DosHeader.e_magic ?
    On W$ecx = 'ZM', jmp L1> ; Is it the older version of DosHeader.e_magic ?
    ...
    mov ecx D$esi+64 ; offset image_base.bytes_in_last,
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase | add ecx D$UserPeStart
    On W$ecx-2 = 'MZ', jmp L1> ; Is it DosHeader.e_magic ?
    On W$ecx-2 = 'ZM', jmp L1> ; Is it the older version of DosHeader.e_magic ?
        ExitP ; This is not the DosHeader.e_magic, Exit
    L1:


    mov ecx D$esi+85 ; offset GetProcAddress,
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase
    add ecx D$SectionsMap
    ; If the pointer was not flaged with IMPORT, exit
...
Proc KillDelayFunction01:
    Arguments @Addresse
    uses esi, ecx, edi, edx
    ...
    add edi D$SectionsMap
;;
    Flag the whole code that contains 10 bytes
...
;;
    ...
    .If B$esi+5 = 0E9
        mov ecx 10
        Do | mov B$edi KILLFLAG | inc edi | dec ecx | Loop_Until ecx = 0

        ; Go to the adrress of the jmp
        mov ecx D$esi+6
        add esi 5 ; Point to jmp Code0763829DC
...
;;
    Flag the code below that contains 17 bytes
...
Code0763829DC: G0:
    ...
    push eax                          ; 50 
    push Data0763AEC34                ; 68 34 EC 3A 76 Pointer to the start of IMAGE_DELAY_DESCRIPTOR
    call Code076382A3C                ; E8 53 00 00 00 
    pop edx                           ; 5A 
    pop ecx                           ; 59 
    jmp eax                           ; FF E0 
...
;;
  Flag the code below that contains 109 bytes
...
Code07E43D09A: J0:
    ...
    push edx                          ; 52 
    push eax                          ; 50 
    push 07E46E34C                    ; 68 4C E3 46 7E  Pointer to the start of IMAGE_DELAY_DESCRIPTOR
    call Code07E43D0B0                ; E8 09 00 00 00 ; The main delayLoadHelper function
    pop edx                           ; 5A 
    pop ecx                           ; 59 
    jmp eax                           ; FF E0 
...
;;
    ...
    ; Follow call Code07E43D0B0

    mov ecx D$esi+1 ; Bypass the 0E8 (The 1st byte of the call instruction
    ; Go to the address of the next code

    If ecx s 0 ; Is ecx signed ? (Negative number)
        add ecx 5 ; go to the end of the call Code07E43D0B0 ; E8 53 00 00 00 we are at the last 0FF
        neg ecx
        sub esi ecx
    Else
        add ecx 5 ; go to the end of the call Code07E43D0B0 ; E8 53 00 00 00 we are at the last 0FF
...
Code07E43D0B2: L4:
    ...
    sub edi D$eax+0C                  ; 2B 78 0C 
    mov eax D$eax+010                 ; 8B 40 10 
    mov ecx 07E410000                 ; B9 00 00 41 7E  Points to the Start of the File 'MZ' DosHeader.e_magic
    sub edi ecx                       ; 2B F9 
    sar edi 02                        ; C1 FF 02 
    add esi ecx                       ; 03 F1 
    mov edx D$esi                     ; 8B 16 
    add ebx ecx                       ; 03 D9 
    lea eax D$eax+edi*4+07E410000     ; 8D 84 B8 00 00 41 7E   Points to the Start of the File 'MZ' DosHeader.e_magic
    mov ecx D$eax                     ; 8B 08 
    test ecx ecx                      ; 85 C9 
     | js N1>  ; Code07E43D163        ; 78 79 
    lea eax D$ecx+Data07E410002       ; 8D 81 02 00 41 7E  Points to DosHeader.e_cblp
    
Code07E43D0F0: B6:
    test edx edx                      ; 85 D2 
    mov D$ebp-04 eax                  ; 89 45 FC 
     | jne K5>  ; Code07E43D149       ; 75 52 
    push ebx                          ; 53 
    call 'KERNEL32.LoadLibraryA'      ; FF 15 F4 12 41 7E 
    mov edi eax                       ; 8B F8 
    test edi edi                      ; 85 FF 
    mov D$ebp+08 edi                  ; 89 7D 08 
     | je N6>  ; Code07E43D168        ; 74 61 
    push 00                           ; 6A 00 
    push edi                          ; 57 
    push esi                          ; 56 
    call 'KERNEL32.InterlockedCompareExchange'  ; FF 15 C4 12 41 7E 
    mov esi eax                       ; 8B F0 
    test esi esi                      ; 85 F6 
     | jne Code07E44E2EB              ; 0F 85 D0 11 01 00 
    push 08                           ; 6A 08 
    pop ecx                           ; 59 
    lea edi D$ebp-024                 ; 8D 7D DC 
    rep stosd                         ; F3 AB 
    mov eax D$ebp+08                  ; 8B 45 08 
    mov D$ebp-010 eax                 ; 89 45 F0 
    mov eax D$Data07E4717E8           ; A1 E8 17 47 7E 
    test eax eax                      ; 85 C0 
    mov D$ebp-028 024                 ; C7 45 D8 24 00 00 00 
    mov D$ebp-01C ebx                 ; 89 5D E4 
     | jne Code07E44E2DE              ; 0F 85 9E 11 01 00 
    
Code07E43D140: J6:
    cmp D$ebp+08 00                   ; 83 7D 08 00 
     | je N6>  ; Code07E43D168        ; 74 22 
    mov edx D$ebp+08                  ; 8B 55 08 
    
Code07E43D149: K5:
    push D$ebp-04                     ; FF 75 FC 
    push edx                          ; 52 
    call 'KERNEL32.GetProcAddress'    ; FF 15 3C 13 41 7E 
    test eax eax                      ; 85 C0 
     | je N6>  ; Code07E43D168        ; 74 11 
...
Code07E43D163: N1:
    movzx eax W$eax                   ; 0F B7 00 
jmp B6<  ; Code07E43D0F0              ; EB 88 
    
Code07E43D168: N6:
    push D$ebp-04                     ; FF 75 FC 
    push ebx                          ; 53 
    call 'KERNEL32.DelayLoadFailureHook'  ; E8 0E 10 03 00 
jmp L9<  ; Code07E43D157              ; EB E4 
...
;;
   ...
   ; 1st check the initial data
    ...
    mov ecx D$esi+60 ; offset image_base.bytes_in_last,
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase | add ecx D$UserPeStart
    On W$ecx-2 = 'MZ', jmp L1> ; Is it DosHeader.e_magic ?
    On W$ecx-2 = 'ZM', jmp L1> ; Is it the older version of DosHeader.e_magic ?
    ...
    mov ecx D$esi+159 ; offset GetProcAddress,
    call IsAddressInsidePE ecx, D$DisImageBase, D$UserPeStart, D$UserPeEnd | On eax = &FALSE, ExitP
    sub ecx D$DisImageBase
    add ecx D$SectionsMap
    ; If the pointer was not flaged with IMPORT, exit
...
;;
  Flag the code below that contains 179 bytes (Untill the ret 08)
...
Proc IsAddressInsidePE:
    Arguments @AddressValue, @ImageBase, @PEStart, @PEEnd
    Uses esi, edx
; UserPeStart
    mov edx D@PEEnd
    mov esi D@AddressValue
    sub esi D@ImageBase | add esi D@PEStart
    If_And esi >= D@PEStart , esi < D@PEEnd
        mov eax &TRUE
    Else
        xor eax eax
    End_If
...
;L0:
        ...
        If D$eax+SECTION_RVA = 0
            mov D$eax+SECTION_RVASIZE 0 | jmp L3>>
        Else_If D$eax+SECTION_RVASIZE = 0
            mov D$eax+SECTION_RVA 0 | jmp L3>>
        End_If

        mov ecx D$eax+SECTION_RVASIZE, ebx D$eax+SECTION_FILESIZE
        Align_On_Variable D$DisRvaSectionAlignment ecx
        Align_On_Variable D$DisFileSectionAlignment ebx
        .If ecx > ebx
            Test_If D$eax+SECTION_FLAG &IMAGE_SCN_MEM_EXECUTE; | jz L2>
                mov B$VirtualIsRunable &TRUE            ; See this pb later.
            Test_End
;L2:
            mov edi D$SectionsMap | add edi D$eax+SECTION_RVA | add edi D$eax+SECTION_FILESIZE
...
FlagTrueDataSection:
    ...
    If B$ForcedataSectiontoData = &TRUE
        call FlagFullTrueDataSection
    Else
        call FlagLimitedDataSection
    End_If

ret

; Convert all data sections to pure data (Respecting, eventual REsources, Imports, Export Flags, in the middle of the section)

FlagFullTrueDataSection:
    mov ecx D$DisNumberOfSections
...
L0: mov ebx D$eax+SECTION_FLAG | test ebx &IMAGE_SCN_MEM_EXECUTE | jnz L8>>
    ...
    ...If ebx = &IMAGE_SCN_CNT_INITIALIZED_DATA__&IMAGE_SCN_MEM_READ__&IMAGE_SCN_MEM_WRITE
        push eax, ecx
          ...
          ; but there are cases where a true data section may contains exported data or imported data
          ; or resources and so on that where pointed by the DataDirectory Section of the PE
          ; and also, eventually, if a byte was incorrectly flagged as Code, we need to reflag it as data.
          ; In all cases we must not touch any of the previously settled flags:VIRTUALFLAG, IMPORTFLAG,
          ; RESOURCESFLAG etc.
            .While esi < ebx
                If B$esi = 0 ; Only mark as data a byte that have no flag at all
                    mov B$esi DATAFLAG
                Else
                    ; In any other cases, see if eventually was flagged as code, we need to reset is as data, for precaution
                    Test_If B$esi CODEFLAG
                        mov B$esi DATAFLAG
                    Test_End
                End_If
                inc esi
            .End_While
            ; also, all the rest is virtual
            While esi < edx
...
; flag all data section when dataflag is found
FlagLimitedDataSection:
    mov ecx D$DisNumberOfSections
