...
L5: .If esi < D$RealDataChunkEdx
        mov edx D$RealDataChunkEdx
            ; Updated in 13/02/2019
            ; this happens when the FPU find a Bad/Subnormal number. We inserted a comment and after it a CRLF.
            ; This helps to distinguish if we have repetitive bytes and the end of it does not contains the same data Type. Ex:
            ; T$ 1236.e-4965 ; Found Bad number (subnormal/imprecise number). Bytes correspondent are: B$ 0FF ....... < insert a CRLF here
            ; T$ 1236.e-4948 ; Found Bad number (subnormal/imprecise number). Bytes correspondent are: B$ 0FF ....... < insert a CRLF here
            ; B$ 0, 0, 2, 2  The next bytes are not a T$ so we insert it to be on the same line and avoid adding extra CRLF
            ; See: WriteDisFP10, WriteFP10, WriteDisFP8, WriteDisFP4
        If W$edi-2 = CRLF
            sub edi 2
...
L0: mov eax edx | sub eax esi
    ...
    ..If esi < edx
        .If edi > D$NextDataOutputBreak
            ...
            ; T$ 1236.e-4965 ; Found Bad number (subnormal/imprecise number). Bytes correspondent are: B$ 0FF ....... < insert a CRLF here
            ; T$ 1236.e-4948 ; Found Bad number (subnormal/imprecise number). Bytes correspondent are: B$ 0FF ....... < insert a CRLF here
            ; B$ 0, 0, 2, 2  The next bytes are nopt a T$ so we insert it to be on the same line and avoid adding extra CRLF
            ; See: WriteFP10
            If W$edi-2 = CRLF
                sub edi 2
            End_If
